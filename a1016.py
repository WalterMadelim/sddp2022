...
def lgf(x,dR,ite,t,al):
    return -cacase(dR,ite,t,al,2,x)[0]
...
# pipe parameters
ppKf = 1.016 # ppK = ppKf * (km) * (m) ** -5
l = np.array([3.7,.77,.521,.521,.521,.77]) # pipe length (km)
d = np.array([.3,.25,.25,.25,.25,.25]) # pipe diameter (m)
ppK = ppKf * l * d ** -5
...
# experimental data for centrifugal pump
...
# cross sectional area of water tank (m2)
tkCoef = 6/160
pipeNum = 6
pumpNum = 2
hrdNum = 3
...
def cacase(dR,ite,t,al,s,pai):
    ...
    for u in range(U_chp):
        ...
        m.addConstr(P_chp[u] - pD_chp - (pD_chp - pC_chp) / (hD_chp - hC_chp) * (h_chp[u] - hD_chp) >= -2 * (1 - mu_chp[u]))
        m.addConstr(P_chp[u] - pB_chp - (pC_chp - pB_chp) / (hC_chp - hB_chp) * (h_chp[u] - hB_chp) >= 0)  # -5e3 * (1 - mu_chp[t,u]))
    # ------------------------------
    Ma, Mh = m.addVars(B, vtype=GRB.CONTINUOUS if s == 1 else GRB.BINARY,ub=1),m.addVars(B, vtype=GRB.CONTINUOUS if s == 1 else GRB.BINARY,ub=1)
    ...
    htin = m.addVar()
    htou = m.addVars(B, vtype=GRB.CONTINUOUS if s == 1 else GRB.BINARY, ub=1)  # discretized state variable
    m.addConstr( (gsum(fb[i]*z[2*B+i] for i in range(B)) if t > 0 else 35) + tkCoef * (q_pipe[2] - q_pipe[3]) - gdot(fb,htou) == [-1/2,1/2] )
    ...
    if t == nss-1: # penalty for not-satisfying daily balance
        ...
        m.addConstr(endPen1 >= (Mamin + Mamax) / 2 - (Mamin + MaSCL*(gdot(fb,Ma))))
        m.addConstr(endPen2 >= (Mhmin + Mhmax) / 2 - (Mhmin + MhSCL*(gdot(fb,Mh))))
        m.addConstr(endPen3 >= 35-gdot(fb,htou))
        theObj += endPen1+endPen2+MaSCL*endPen3

    tha = m.addVar()
    if t < nss-1:
        for its in range(ite+dR):
            m.addConstr(tha >= cut[its, t,  -1] + gsum(cut[its, t,  i] * Ma[i] for i in range(B)) + gsum(
                    cut[its, t,  B + i] * Mh[i] for i in range(B)) + gsum(
                    cut[its, t,  2 * B + i] * htou[i] for i in range(B)) + gsum(
                    cut[its, t,  3 * B + i] * mu_chp[i] for i in range(U_chp)))
    theObj += tha
    # -----model part-----
    if s == 2:
    ...
    if m.status != GRB.OPTIMAL:
        ...
        exit(3)
    if dR == 0:
        Mathis,Mhthis = np.zeros(B,dtype=np.int8),np.zeros(B,dtype=np.int8)
        muchpthis = np.zeros(U_chp,dtype=np.int8)
        htouthis = np.zeros(B,dtype=np.int8)
        for i in range(B):
